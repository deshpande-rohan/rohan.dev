...
export const Button = styled(Link)`
    ...
    background: ${({ sharpBlack }) => sharpBlack ? '#101010' : 'linear-gradient(90.27deg, #61E87F 3.75%, rgba(132, 244, 177, 0.42) 99.77%)'};
    border-radius: ${({ rounded }) => rounded ? '50px' : '4px'};
    white-space: nowrap;
    color: ${({ dark }) => dark ? 'rgba(255, 255, 255, 0.7)' : '#FFFFFF'};
    font-size: ${({ normalFont }) => normalFont ? '22px' : '28px'};
    padding: ${({ normalPadding }) => normalPadding ? '12px 24px' : '15px 26px'};
    outline: none;
    border: none;
    cursor: pointer;
    display: flex;
